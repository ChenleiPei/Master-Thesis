...
def calculate_log_likelihood_from_gaussian(points_file, target_func, std):
    """
    ...
    log_likelihoods = norm.logpdf(y_observed, loc=y_mean, scale=std)
    #devide the number of points
    joint_log_likelihood = -(np.sum(log_likelihoods) / len(log_likelihoods)) # Sum of log-likelihoods
     # Sum of log-likelihoods
    joint_log_likelihood = np.log10(joint_log_likelihood)
...
if __name__ == "__main__":
    ...
    log_likelihood = calculate_log_likelihood_from_gaussian(filename, target_function, std=1)
